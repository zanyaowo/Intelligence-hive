...
class Api:
    ...
    async def return_snare_info(self, uuid, count=-1):
        query_res = []
        try:
            # aioredis 2.x uses 'start' instead of 'offset' and 'num' instead of 'count'
            if count == -1:
                query_res = await self.redis_client.zrevrangebyscore(uuid, '+inf', '-inf')
            else:
                query_res = await self.redis_client.zrevrangebyscore(uuid, '+inf', '-inf', start=0, num=count)
        except aioredis.exceptions.ConnectionError as connection_error:
            self.logger.exception("Can not connect to redis %s", connection_error)
        else:
            if not query_res:
